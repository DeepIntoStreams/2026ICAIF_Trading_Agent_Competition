...
CREATE TABLE IF NOT EXISTS team_api_credentials (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Upgrade existing single-key team rows without invalidating their credentials.
INSERT INTO team_api_credentials
       (team_id, key_hash, status, issued_at, created_at)
SELECT id, api_key_hash, 'ACTIVE', created_at, created_at
  FROM teams
...
-- A missing provider row is never sufficient evidence of a halt. Only an
-- explicit, attributable full-session event may make one instrument
-- non-tradable without rejecting the complete market day.
CREATE TABLE IF NOT EXISTS instrument_market_events (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    trading_day_id BIGINT NOT NULL REFERENCES trading_days(id),
    instrument_id BIGINT NOT NULL REFERENCES instruments(id),
    event_type TEXT NOT NULL CHECK (event_type IN (
        'FULL_SESSION_HALT',
        'FULL_SESSION_NO_TRADING',
        'OTHER_CONFIRMED_NO_TRADING'
    )),
    trading_status TEXT NOT NULL CHECK (trading_status = 'NON_TRADABLE'),
    confirmation_source TEXT NOT NULL CHECK (length(trim(confirmation_source)) > 0),
    confirmation_reference TEXT NOT NULL CHECK (length(trim(confirmation_reference)) > 0),
...
CREATE INDEX IF NOT EXISTS market_events_by_instrument_day
    ON instrument_market_events(instrument_id, trading_day_id);
...
CREATE TABLE IF NOT EXISTS market_bars (
    ...
    market_event_id BIGINT REFERENCES instrument_market_events(id),
...
-- Repeat-safe upgrade for databases created before per-instrument market
-- status and source-consensus results were retained.
...
ALTER TABLE market_bars
    ADD COLUMN IF NOT EXISTS market_event_id BIGINT;
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
         WHERE conrelid='market_bars'::regclass
           AND conname='market_bars_market_event_id_fkey'
    ) THEN
        ALTER TABLE market_bars
            ADD CONSTRAINT market_bars_market_event_id_fkey
            FOREIGN KEY (market_event_id) REFERENCES instrument_market_events(id);
    END IF;
END $$;
...
-- Every provider response used by the verifier is retained separately from
-- the official consensus row. This makes source disagreement and later
-- corrections auditable without mutating the competition input.
CREATE TABLE IF NOT EXISTS market_bar_candidates (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    trading_day_id BIGINT NOT NULL REFERENCES trading_days(id),
    instrument_id BIGINT NOT NULL REFERENCES instruments(id),
    source TEXT NOT NULL,
    ...
    received_at TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL,
    UNIQUE (trading_day_id, instrument_id, source),
    CHECK (high >= low),
    CHECK (high >= open AND high >= close),
    CHECK (low <= open AND low <= close)
);

CREATE INDEX IF NOT EXISTS market_candidates_by_day_source
    ON market_bar_candidates(trading_day_id, source, instrument_id);
...
-- Repeat-safe upgrade for databases created before filing versions were
-- retained. Legacy rows remain identifiable and no timestamp is invented.
ALTER TABLE fundamental_records ADD COLUMN IF NOT EXISTS filing_id TEXT;
UPDATE fundamental_records
   SET filing_id='legacy:' || period_end::TEXT || ':' || id::TEXT
 WHERE filing_id IS NULL;
ALTER TABLE fundamental_records ALTER COLUMN filing_id SET NOT NULL;
ALTER TABLE fundamental_records
    DROP CONSTRAINT IF EXISTS fundamental_records_instrument_id_period_end_source_key;
CREATE UNIQUE INDEX IF NOT EXISTS fundamentals_by_filing
    ON fundamental_records(instrument_id, source, filing_id);
...
CREATE TABLE IF NOT EXISTS portfolio_snapshots (
    ...
    execution_id BIGINT,
    prior_close_snapshot_id BIGINT,
    ...
    CONSTRAINT portfolio_snapshots_prior_close_fkey
        FOREIGN KEY (prior_close_snapshot_id) REFERENCES portfolio_snapshots(id),
    CONSTRAINT portfolio_snapshots_prior_close_shape CHECK (
    ...
    CONSTRAINT portfolio_snapshots_prior_close_not_self CHECK (
        prior_close_snapshot_id IS NULL OR prior_close_snapshot_id <> id
    )
);
...
CREATE TABLE IF NOT EXISTS leaderboard (
    ...
    avg_rank NUMERIC(28, 12) NOT NULL CHECK (avg_rank > 0),
    m1_cumulative_return NUMERIC(28, 12),
    m2_daily_win_rate NUMERIC(28, 12),
    m3_sharpe_ratio NUMERIC(28, 12),
    m4_sortino_ratio NUMERIC(28, 12),
    m5_maximum_drawdown NUMERIC(28, 12),
    m6_value_at_risk_95 NUMERIC(28, 12),
    m7_expected_shortfall_95 NUMERIC(28, 12),
    m8_turnover NUMERIC(28, 12),
    m9_violation_rate NUMERIC(28, 12),
    dimension_scores JSONB NOT NULL,
    calculated_at TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL,
    UNIQUE (trading_day_id, team_id),
    UNIQUE (trading_day_id, rank)
);

CREATE INDEX IF NOT EXISTS leaderboard_by_day_rank
    ON leaderboard(trading_day_id, rank);
